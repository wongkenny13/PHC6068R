...
imageClassifier <- function(url = "https://res.cloudinary.com/twenty20/private_images/t_low-crop/photosp/f985a04a-3f6a-4407-b3d8-722aac5eb6d8/f985a04a-3f6a-4407-b3d8-722aac5eb6d8.jpg",
    ...
    imgRGB <- data.frame(x = rep(1:imgDm[2], each = imgDm[1]),
        y = rep(imgDm[1]:1, imgDm[2]), R = as.vector(img[,
            , 1]), G = as.vector(img[, , 2]), B = as.vector(img[,
            , 3]))
    # Plot the image
    ...
    plot1 <- ggplotshort(rgb(imgRGB[c("R", "G", "B")]), imgRGB,
        "Original", themevals)
    if (numclust%%1 != 0 || numclust < 2) {
        ...
        numclust <- as.numeric(which.max(avg_sil_values) +
            1)
    }
    kMeans <- kmeans(imgRGB[, c("R", "G", "B")], centers = numclust,
        nstart = 20)
    kColors <- rgb(kMeans$centers[kMeans$cluster, ])
    plot2 <- ggplotshort(kColors, imgRGB, paste("K-Means Clustering of",
        numclust, "Colors"), themevals)
    ggplot2::ggsave("kmeanscomparison.jpg", gridExtra::arrangeGrob(plot1,
        plot2, ncol = 2, nrow = 1))
    wd <- getwd()
